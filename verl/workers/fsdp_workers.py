...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if self._is_actor:
            ...
            if self.config.actor.ppo_micro_batch_size_per_gpu is not None:
                assert (
                    self.config.actor.ppo_mini_batch_size % self.config.actor.ppo_micro_batch_size_per_gpu
                    == 0
                ), (
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! start modify
            ...
            rollout = KTRollout(module=self.actor_module_fsdp, config=self.config.rollout)
            rollout_sharding_manager = BaseShardingManager()
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! end modify
            ...
            try:
                if fsdp_version(self.actor_module_fsdp) > 0:
                    ...
                    if dist.get_rank() == 0:
                        ...
                        with open(
                            os.path.join(lora_save_path, "adapter_config.json"), "w", encoding="utf-8"
                        ) as f:
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # pad it back
                rm_score = pad_input(reward_rmpad, indices=indices, batch=batch_size, seqlen=seqlen).squeeze(
                    -1
                )
            else:
                output = self.reward_module(
