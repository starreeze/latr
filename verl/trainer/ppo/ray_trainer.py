    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if config.get("use_pf_ppo", False):
            data = core_algos.compute_pf_ppo_reweight_data(
                data, config.pf_ppo.reweight_method, config.pf_ppo.weight_pow
            )
...
class RayPPOTrainer:
    ...
    def _validate_config(self):
        ...
        if config.actor_rollout_ref.actor.strategy == "megatron":
            ...
            assert (
                n_gpus % (model_parallel_size * config.actor_rollout_ref.actor.megatron.context_parallel_size)
                == 0
            ), (
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # collect metrics
                batch.batch["prompts_texts"] = self.tokenizer.batch_decode(
                    batch.batch["prompts"], skip_special_tokens=True
                )
