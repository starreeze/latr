...
class KTBase(PreTrainedModel, GenerationMixin):
    pass


class KeyTokenGenMixin(KTBase if TYPE_CHECKING else object):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.filter_model = None
        self.sched: BranchParamScheduler = None  # type: ignore
    ...
    def generate(
        ...
        **kwargs,
        ...
        """
        if config is not None and config.kt_fallback_mode or kwargs.get("kt_fallback_mode", False):
        ...
        kwargs_config = init_dataclass_from_dict(KeyTokenGenConfig, kwargs)
        if config is None:
            config = kwargs_config
        else:
            config.__dict__.update(kwargs_config.__dict__)
        if self.sched is None:
            self.sched = init_dataclass_from_dict(BranchParamScheduler, config.__dict__)
        ...
        key_token_filters = create_key_token_filter(
            ...
            prob_filter_thres=self.sched.prob_filter_thres,
            similarity_filter_thres=self.sched.similarity_filter_thres,
            stop_word_filter=config.stop_word_filter,
        )
        if config.model_filter_path is not None and self.filter_model is None:
            self.filter_model = DivJudge(config.model_filter_path)
            self.filter_model.to(input_ids.device)
            if self.sched.model_filter_cand_thres is not None:
                key_token_filters.append(
                    ModelFilter(tokenizer, self.filter_model, self.sched.model_filter_cand_thres)
                )
        ...
        rollout_filter = RolloutFilter(
            ...
            self.sched.rollout_filter_edit_dist_thres,
            self.sched.model_filter_rollout_thres,
            tokenizer,
            self.filter_model if self.sched.model_filter_rollout_thres is not None else None,
        )
        ...
        for step in wrapped_range:
            ...
            current_ids, current_attention_mask, cache = self._sample_new_sequence_with_rollout(
                key_token_filters,
                probs,
                current_ids,
        ...
        self.sched.step(suppress_ratio, empty_branch_ratio)
